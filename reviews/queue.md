...
- Local `resources` legacy gate is green: `121/121` CTest tests pass according to `test-harness-status.md`.
- Local `scheduler` legacy gate is classified, not green, because `_scheduler` is not built on this Linux host and upstream presets are Windows/macOS oriented.
- No Rust migration crates are present in this checkout, so all Rust implementation statuses are `open` or `blocked`.
