...
| --- | --- | --- | --- | --- | --- |
| tasklet lifecycle | `PyTasklet_New`, `PyTasklet_Setup`, `PyTasklet_Insert`, `PyTasklet_Alive`, `PyTasklet_Kill` | Represent tasklet IDs, alive/dead state, bound operation, scheduled/paused/blocked flags, insertion/removal, kill before run, kill while blocked. `CoreScheduler` now exposes tasklet runtime snapshots, explicit pause/resume transitions, and schedule-clears-paused behavior; the live PyO3 bridge mirrors alive/scheduled/paused/times_switched_to for covered setup/run/finish/block/continuation/clear/kill paths and uses core paused snapshots for direct-run paused eligibility. | Emit `tasklet.new`, `tasklet.setup`, `tasklet.insert`, `tasklet.kill`, `tasklet.complete`, and state snapshots. | Map Rust tasklets to `PyTaskletObject*`, Python callable setup, Python args/kwargs, and return codes; make core snapshots authoritative for scheduling/lifecycle decisions. | Python refcount behavior, weakrefs, cyclic callable cleanup, `TaskletExit` exception injection, tracer/context-manager behavior, and broader Greenlet continuation ownership. |
...
| `test_channel.py::*refcount*`, `test_*cleanup_on_thread_finish`, `test_inter_thread_communication`, `test_nested_channel_with_parent_death_running_fine_and_cleaning_up_correctly` | core/ffi split | `fixtures/scheduler/channel_kill_error_cleanup_while_blocked.json`, `fixtures/scheduler/channel_close_cleanup_blocked_receivers.json`, plus Python bridge tests | Core now covers blocked sender/receiver close cleanup; Python object lifetime, GC, threads, and Greenlet parent behavior remain in the bridge. |
...
45 JSON fixtures: 44 with ordered event expectations and one final-state
...
legacy binaries are `resources_debug` with `CMAKE_BUILD_TYPE=Debug`, so the
rows remain observed-only even though Rust is running `release-native`.
