...
block-trap no-mutation checks. It also exposes core-owned kill/pending-exit state
for immediate kill, pending insertion, blocked-channel cleanup, payload-token
removal, and dead-tasklet no-op behavior. Live PyO3 tasklet/channel objects now carry opaque
...
queue identity adapters, callbacks, and broader lifecycle transitions while the
bridge keeps only Python callables, exceptions, and PyObject payload storage.
...
| `test_tasklet.py::test_run`, `test_run_args`, `test_run_args_kwargs`, `test_remove_and_run`, `test_remove_and_switch`, `test_paused`, `test_kill_tasklet`, `test_times_switched_to*` | core then ffi | `fixtures/scheduler/tasklet_lifecycle_remove_insert_switch_times.json`, `fixtures/scheduler/tasklet_kill_pending_and_immediate.json`, plus unchanged Rust PyO3 bridge subset for direct run/args/kwargs | Core fixtures cover remove/insert/switch counts and kill pending/immediate; the CoreScheduler handle API covers immediate kill, pending-exit scheduling, dead no-op, and blocked-channel cleanup. Python bridge covers direct run, argument passing, core-owned pause/resume snapshots for bind/remove/insert, and direct-run paused eligibility. Remaining Greenlet continuation semantics are not report-ready. |
