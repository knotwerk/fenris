...
per-run-queue current-tasklet identity, per-run-queue scheduler switch-trap
depth, explicit pause/resume lifecycle
transitions, balance, blocked queues, preference clamping, close/open/clear, and
block-trap no-mutation checks. It also exposes core-owned kill/pending-exit and
tasklet main identity, tasklet parent identity, tasklet callable-bound state,
tasklet bound-argument state, tasklet dont_raise state, tasklet highlighted
state, tasklet context state, tasklet method/module/file/line/parent-callsite
metadata, tasklet timing metrics, tasklet continuation/kill delivery state, and
tasklet channel-callback skip state for immediate kill, pending insertion,
blocked-channel cleanup, payload-token
removal, dead-tasklet no-op, timeout completion, and paused direct-run behavior. Live PyO3 tasklet/channel objects now carry opaque
`CoreTaskletId`/`CoreChannelId` handles and route unbuffered channel balance,
preference, block-trap, and blocked sender/receiver queue transitions through
`CoreScheduler` with no bridge-local Rust channel blocked sender/receiver queue
fields or channel balance, preference, closing, or closed fields; covered
unbuffered send/receive transfers now consume
...
`CoreScheduler::queue_front` IDs back to legacy Python tasklet objects. Covered
channel payload handoff identity now uses core-owned `CorePayloadToken` values;
the PyO3 bridge stores the actual Python value/exception objects in a
token-keyed registry outside channel objects for legacy compatibility. Blocked
send/receive throw and cleanup paths now resolve
...
The PyO3 tasklet wrapper no longer keeps bridge-local Rust `blocked`,
block-trap, callable-bound, or args-bound booleans; public `tasklet.blocked`
and blocked run/switch guards read the `CoreScheduler` `blocked_on` snapshot
directly, `block_trap` getters/setters read and write `CoreScheduler`
snapshots directly, and bound-argument guards read `CoreScheduler` snapshots
while the bridge keeps only the Python args/kwargs payloads needed for callable
execution.
It also no longer keeps bridge-local Rust timing metric fields; tasklet
execution start/finish and the `runTime` setter write `CoreScheduler` timing
snapshots directly.
Live PyO3 tasklet objects also mirror
alive/scheduled/paused/times_switched_to plus startTime/endTime/runTime,
dont_raise, highlighted, context, method/module/file/line/parent-callsite
metadata, callable-bound, and bound-argument state through `CoreScheduler` tasklet
...
callable execution, public Python scheduler `getruncount()`/
`calculateruncount()` and C API `PyScheduler_GetRunCount` now read
CoreScheduler runnable queue counts instead of bridge-local PyObject registry
length, public `getcurrent()`/`tasklet.is_current` and C API
`PyScheduler_GetCurrent` now resolve through CoreScheduler per-run-queue
current-tasklet identity, public Python/C API active/all-time tasklet counters now read
CoreScheduler lifetime counters instead of PyO3 statics, PyO3 tasklets no longer
keep a bridge-local counted-active flag for destructor retirement, public Python/C API
active schedule-manager counters now read CoreScheduler counters instead of
PyO3 statics, public Python/C API active channel counters now read
CoreScheduler active-channel counters instead
of PyO3 statics, and C API last-timeout completed/switched counters now
read CoreScheduler timeout counters instead of PyO3 statics, and public
nested-tasklet mode now reads CoreScheduler state instead of PyO3 statics,
public schedule callback registration state, schedule-callback dispatch
metadata for previous/next switch points, and per-run-queue channel callback
registration state now read CoreScheduler state instead of PyO3 atomics, and Python scheduler switch-trap entrypoints plus
trapped operation guards now use CoreScheduler per-run-queue state. Python runtime sync
no longer writes the core scheduled flag; Python-visible tasklet
is_current/alive/blocked/scheduled/paused/block_trap/times_switched_to/startTime/endTime/runTime/dont_raise/highlighted/context/method_name/module_name/file_name/line_number/parent_callsite properties and the
public tasklet call/setup/insert/run/switch/bind/unbind/dont_raise invalid-state
guards including callable-bound, bound-argument, block-trap, dont_raise, highlighted, context, callsite metadata, and timing checks, current-tasklet block-trap guard used by channel operations, and C API
times-switched/block-trap/context getters now read CoreScheduler snapshots, including
...
Python-visible channel balance/preference/closing/closed properties also read
CoreScheduler snapshots with no bridge-local Rust channel balance/preference/closing/closed fields. Current-thread runnable PyObject registry storage is
...
PyObject identity adapters, callback callable invocation/storage, and broader
lifecycle transitions while the bridge keeps only Python callables, exceptions,
and PyObject payload storage.
...
stable-current `block_trap`, and CoreScheduler-backed nested-tasklet mode
roundtripping. It is
...
| --- | --- | --- | --- | --- | --- |
| tasklet lifecycle | `PyTasklet_New`, `PyTasklet_Setup`, `PyTasklet_Insert`, `PyTasklet_Alive`, `PyTasklet_Kill` | Represent tasklet IDs, alive/dead state, bound operation, scheduled/paused/blocked flags, insertion/removal, kill before run, kill while blocked. `CoreScheduler` now exposes tasklet runtime snapshots, main-identity snapshots, parent-identity snapshots, callable-bound snapshots, bound-argument snapshots, block-trap snapshots, dont_raise snapshots, highlighted snapshots, context snapshots, callsite metadata snapshots, per-run-queue current-tasklet identity, parent-cycle rejection, tasklet continuation/kill delivery-state snapshots, tasklet channel-callback skip snapshots, explicit pause/resume transitions, schedule-clears-paused behavior, and transient scheduled snapshots for legacy current-tasklet schedule yields; the live PyO3 bridge mirrors current/main identity, parent identity, callable-bound state, bound-argument state, block-trap state, dont_raise state, highlighted state, context state, method/module/file/line/parent-callsite metadata, and alive/scheduled/paused/times_switched_to for covered setup/run/finish/block/continuation/clear/kill paths, but no longer keeps bridge-local Rust blocked, block-trap, callable-bound, or args-bound booleans. Public blocked state and blocked run/switch guards read `CoreScheduler` `blocked_on` snapshots, block_trap getters/setters read and write `CoreScheduler` snapshots, and bound-argument guards read `CoreScheduler` snapshots while PyO3 retains only the actual Python args/kwargs payloads for execution. The bridge uses core snapshots for `PyTasklet_Alive`, call/setup/insert callable-bound guards, `dont_raise` getter/setter and exception suppression, highlighted getter/setter state, context getter/setter state plus `PyTasklet_GetContext`, callsite metadata getters/setter, `dont_raise` bound-argument guards, dead-tasklet kill no-op decisions, timeout completion accounting, and paused direct-run eligibility, and exposes public tasklet identity/lifecycle/scheduling/block-trap/switch-count properties from core snapshots. | Emit `tasklet.new`, `tasklet.setup`, `tasklet.insert`, `tasklet.kill`, `tasklet.complete`, and state snapshots. | Map Rust tasklets to `PyTaskletObject*`, Python callable setup, Python args/kwargs, Python context-manager/exception-handler callables, and return codes; make core snapshots authoritative for scheduling/lifecycle decisions. | Python refcount behavior, weakrefs, cyclic callable cleanup, `TaskletExit` exception injection, tracer/context-manager callable behavior, and broader Greenlet continuation ownership. |
| tasklet properties | `PyTasklet_GetBlockTrap`, `PyTasklet_SetBlockTrap`, `PyTasklet_IsMain`, `PyTasklet_Check`, `PyTasklet_GetTimesSwitchedTo`, `PyTasklet_GetContext` | Track `is_main`, `block_trap`, `times_switched_to`, `context`, and method/module/file/line/parent-callsite metadata in `CoreScheduler` snapshots; model type checks as API-layer concerns. | Emit switch-count changes, main identity snapshots, context snapshots, callsite metadata snapshots, and block-trap rejection events. | Expose exact getters/setters, `PyTasklet_IsMain` snapshot reads, `PyTasklet_Check` semantics, CoreScheduler-backed `GetContext` C-string access, and callsite metric/string setter compatibility. | Python frame inspection and invalid subclass construction behavior. |
| channel basics | `PyChannel_New`, `PyChannel_Send`, `PyChannel_Receive`, `PyChannel_GetBalance`, `PyChannel_GetPreference`, `PyChannel_SetPreference`, `PyChannel_Check` | Implement unbuffered channels, blocked sender/receiver queues, balance rules (`+senders`, `-receivers`), preference clamping to `-1/0/1`, transfer matching, close/open/clear, remove-blocked cleanup, and block-trap no-mutation checks. The `CoreScheduler` handle API covers these as Rust-owned state, and the live PyO3 bridge now routes unbuffered channel balance/preference/block-trap/blocked-queue transitions through `CoreTaskletId`/`CoreChannelId` handles while using core send/receive results and core queue snapshots to select matched Python tasklets by core ID, carry core-owned payload handoff tokens, resolve blocked channel/direction for throw cleanup from core snapshots, decide immediate peer handoff, select `channel.queue` fronts in covered paths, use a `CoreChannelId` keyed channel object identity adapter, store Python value/exception objects in a `CorePayloadToken` bridge registry outside channel objects, and expose public channel balance/preference/closing/closed getters from core snapshots with no bridge-local Rust channel blocked sender/receiver queue fields, payload map, or balance/preference/closing/closed fields. | Emit `channel.new`, `channel.send.attempt`, `channel.receive.attempt`, `channel.block`, `channel.transfer`, `channel.unblock`, `channel.balance`. | Convert Rust channel operations to core-owned payload tokens plus bridge-owned `PyObject*` payload/error registry entries over opaque core handles. | Bridge-owned `CorePayloadToken` payload registry drain/refcount checks, blocked-tasklet lifetime refs, broader PyObject identity adapters, broader scheduling ownership, and exact exception text outside core semantics still need to move behind adapter boundaries. |
| channel errors | `PyChannel_SendException`, `PyChannel_SendThrow` | Model typed error payloads as channel messages once value transfer is green. | Emit exception-transfer events with symbolic error names and args. | Preserve Python exception type/value/traceback behavior. | Full traceback preservation, `sys.exc_info()`, and completed-transfer TaskletExit delivery details. |
...
| scheduler control | `PyScheduler_Schedule`, `PyScheduler_GetRunCount`, `PyScheduler_GetCurrent`, `PyScheduler_RunWithTimeout`, `PyScheduler_RunNTasklets`, `PyScheduler_GetScheduler` | Implement deterministic run queue, current tasklet, `schedule`, `schedule_remove`, full run, limited run, run-count semantics including main tasklet, deadlock detection, CoreScheduler-owned per-run-queue current-tasklet identity, CoreScheduler-owned per-run-queue switch-trap level and no-mutation guards, and wall-clock `RunWithTimeout` pumping that always runs at least one queued tasklet for zero/expired timeouts. Public Python `getcurrent()`/`tasklet.is_current` and C API `PyScheduler_GetCurrent` now resolve through CoreScheduler current-tasklet identity; public Python `getruncount()`/`calculateruncount()` and C API `PyScheduler_GetRunCount` now read CoreScheduler runnable queue counts while the bridge keeps only tasklet object identity for callable execution. | Emit `scheduler.run.start`, `scheduler.switch`, `scheduler.schedule`, `scheduler.schedule_remove`, `scheduler.switch_trap`, `scheduler.switch_trap_error`, `scheduler.deadlock`, and final run counters. | Expose the capsule functions and Python `scheduler` object. | Callback reentrancy, advanced Greenlet exception paths, public Python API edge cases. |
| scheduler callbacks | `PyScheduler_SetChannelCallback`, `PyScheduler_GetChannelCallback`, `PyScheduler_SetScheduleCallback`, `PyScheduler_SetScheduleFastCallback` | Invoke Python schedule callbacks, share schedule callbacks across threads, and invoke the fast C schedule callback at switch points covered by bridge parity tests. | `fixtures/scheduler/scheduler_callback_switch_points.json` emits ordered scheduler callback-point trace events with previous/next tasklet identity and switch reason; schedule callback registration state, schedule-callback dispatch identity/enable metadata, per-run-queue channel callback registration state, and tasklet channel-callback skip state now live in `CoreScheduler`, while user callback callable/function-pointer storage and invocation remain in the Python bridge. | Python-level callback set/get storage, CoreScheduler-backed schedule/channel callback registration state, CoreScheduler-backed schedule-callback dispatch metadata, CoreScheduler-backed tasklet channel-callback skip state, schedule callback basic ordering, multi-thread visibility, simple channel callback order, and fast C callback smoke are covered. | Callback exceptions, callback reentrancy, broader lifetime/refcount cleanup. |
| scheduler counters | `PyScheduler_GetNumberOfActiveScheduleManagers`, `PyScheduler_GetNumberOfActiveChannels`, `PyScheduler_GetAllTimeTaskletCount`, `PyScheduler_GetActiveTaskletCount`, `PyScheduler_GetTaskletsCompletedLastRunWithTimeout`, `PyScheduler_GetTaskletsSwitchedLastRunWithTimeout` | Track active channel count, active/all-time tasklet count, completed count, and switch count. One schedule manager per core instance. CoreScheduler now owns active/all-time tasklet lifetime counter storage, active schedule-manager lifetime counter storage, active channel lifetime counter storage, last-timeout completed/switched counter storage, and public nested-tasklet mode storage; the PyO3 bridge only signals tasklet/schedule-manager/channel construction/destruction, RunWithTimeout progress, and nested-mode API calls. Tasklet counted/active retirement is decided by CoreScheduler, not a bridge-local counted-active flag. | Include final counter assertions in fixtures where relevant. | Export exact integer-returning C API; `PyScheduler_GetAllTimeTaskletCount`/`PyScheduler_GetActiveTaskletCount`/`PyScheduler_GetNumberOfActiveScheduleManagers`/`PyScheduler_GetNumberOfActiveChannels`/`PyScheduler_GetTaskletsCompletedLastRunWithTimeout`/`PyScheduler_GetTaskletsSwitchedLastRunWithTimeout` read CoreScheduler counters, and `schedule_manager_wrapper_lifecycle_matches_legacy_thread_cache` covers cached manager identity, weakrefs, `sys.getrefcount(... ) == 2`, and active-manager teardown across a thread. | Monotonic timeout policy and broader Python GC timing remain bridge-owned or adapter-owned outside the covered tasklet/schedule-manager/channel/timeout counter storage slices. |

## Legacy Support File Parity Notes

| Support source | Public behavior affected | Rust guard |
| --- | --- | --- |
| `Utils.cpp`/`Utils.h::StdStringFromPyObject` | `tasklet.context` and `tasklet.parent_callsite` accept only Python `str` and reject non-strings with `TypeError("value must be a string")`; both values are stored in `CoreScheduler` snapshots. `Tasklet::SetCallsiteData` also uses the helper internally for method/module/file metrics after coercing `__name__` and `__module__` through `str()`, then writes that metadata to CoreScheduler. | `carbon-scheduler-rs/crates/carbon-scheduler-python/src/lib.rs::tasklet_getset_properties_match_legacy_py_tasklet_surface` checks metric extraction, string round-trips, and exact non-string setter error text; `py_tasklet_context_getter_reads_core_snapshot` proves context drift follows CoreScheduler, and `py_tasklet_callsite_metadata_getters_read_core_snapshot` proves method/module/file/line/parent-callsite drift follows CoreScheduler. |
...
| `test_scheduler.py::test_scheduler_run_order`, `test_tasklet_run_order`, `test_tasklet_run_order_2`, blocked/dead direct tasklet run/switch paths, `test_utils.py::TestWithLimit`, `capiTest/Scheduler.cpp::PyScheduler_RunNTasklets` | core | `fixtures/scheduler/run_order.json`, `fixtures/scheduler/scheduler_run_n_tasklets_one_fifo_boundary.json`, `fixtures/scheduler/scheduler_run_n_tasklets_limited_nested_schedule_order.json`, `fixtures/scheduler/scheduler_run_n_tasklets_limited_schedule_order_no_nested.json`, `fixtures/scheduler/scheduler_run_n_tasklets_limited_multi_level_nested_schedule_order.json`, `fixtures/scheduler/scheduler_run_n_tasklets_limited_multi_level_schedule_order_no_nested.json`, `fixtures/scheduler/tasklet_run_nested_target_first.json`, `fixtures/scheduler/tasklet_run_nested_target_middle.json`, `fixtures/scheduler/tasklet_run_no_nested_target_middle.json`, `fixtures/scheduler/tasklet_invalid_run_switch_no_mutation.json` | FIFO runnable order, bounded one-tasklet scheduler pumping, limited schedule-order pumping in nested and non-nested modes, targeted `tasklet.run()` ordering including the non-nested FRONT_PLUS_ONE boundary, and invalid direct run/switch no-mutation behavior. |
| `test_scheduler.py::test_nested_tasklet_run_order`, `test_nested_tasklet_run_order_with_schedule`, `test_multi_level_nested_tasklet_run_order_with_schedule`, `test_multi_level_nested_tasklet_run_order_with_yield_to_blocked` | core | `fixtures/scheduler/nested_tasklet_run_order.json`, `fixtures/scheduler/nested_tasklet_run_order_no_nested.json`, `fixtures/scheduler/nested_tasklet_schedule_order.json`, `fixtures/scheduler/nested_tasklet_schedule_order_no_nested.json`, `fixtures/scheduler/multi_level_nested_schedule_order.json`, `fixtures/scheduler/multi_level_nested_schedule_order_no_nested.json`, `fixtures/scheduler/scheduler_schedule_back_reschedule_order.json`, `fixtures/scheduler/nested_yield_to_blocked.json`, `fixtures/scheduler/nested_yield_to_blocked_no_nested.json`, `fixtures/scheduler/nested_tasklet_parent_chain.json`, `fixtures/scheduler/nested_tasklet_switch_parent_chain.json`, `fixtures/scheduler/nested_tasklet_switch_blocked_parent_chain.json`, plus the four `scheduler_run_n_tasklets_limited_*schedule_order*.json` fixtures | Single-level, multi-level, blocked-yield, BACK requeue, parent/depth, parent-chain switch/resume, blocked parent-chain switch unwind, and limited-pump nested tasklet variants are covered for nested and non-nested modes where applicable. |
...
| `test_channel.py::test_set_channel_callback`, `test_channel_callback_with_blocking_send`, `test_channel_callback_with_blocking_receive` | core then ffi/partial | `fixtures/scheduler/channel_callback_will_block.json` plus bridge tests | Core records callback will-block points, schedule-callback dispatch metadata, callback registration state, and the tasklet one-shot channel-callback skip decision; Python-level callback callable/function-pointer storage and invocation order for blocking send/receive are covered in the bridge tests. Broader callback exception/reentrancy behavior remains open. |
...
| `test_tasklet.py::test_run`, `test_run_args`, `test_run_args_kwargs`, `test_remove_and_run`, `test_remove_and_switch`, `test_paused`, `test_kill_tasklet`, `test_times_switched_to*` | core then ffi | `fixtures/scheduler/tasklet_lifecycle_remove_insert_switch_times.json`, `fixtures/scheduler/tasklet_times_switched_to_receive_schedule_bind_reset.json`, `fixtures/scheduler/tasklet_kill_pending_and_immediate.json`, `fixtures/scheduler/nested_tasklet_switch_parent_chain.json`, `fixtures/scheduler/nested_tasklet_switch_blocked_parent_chain.json`, plus unchanged Rust PyO3 bridge subset for direct run/args/kwargs | Core fixtures cover remove/insert/switch counts, receive/schedule/rebind switch-count behavior, nested switch/resume counts, blocked switch-chain unwind after channel wakeup, and kill pending/immediate; the CoreScheduler handle API covers parent identity, parent-cycle rejection, immediate kill, pending-exit scheduling, dead no-op, blocked-channel cleanup, continuation/kill delivery-state snapshots, and channel-callback skip snapshots. Python bridge covers direct run, argument passing, core-owned parent identity, core-owned pause/resume snapshots for bind/remove/insert, `PyTasklet_Alive`, dead-tasklet kill no-op, timeout completion, and paused direct-run eligibility from core snapshots. Remaining Greenlet continuation semantics are not report-ready. |
...
| `test_scheduler.py::test_schedule`, `test_schedule_remove_fail`, `TestRun::test_calling_run_from_non_main_tasklet`, `TestSwitch::*`, `TestSwitchTrap::*` | core then ffi/deferred | `fixtures/scheduler/nested_tasklet_schedule_order.json`, `fixtures/scheduler/nested_tasklet_schedule_order_no_nested.json`, `fixtures/scheduler/scheduler_schedule_back_reschedule_order.json`, `fixtures/scheduler/schedule_remove_reinsert_paused_tasklet.json`, `fixtures/scheduler/nested_tasklet_switch_parent_chain.json`, `fixtures/scheduler/nested_tasklet_switch_blocked_parent_chain.json`, `fixtures/scheduler/scheduler_switch_trap_no_state_mutation.json` | Schedule order, `scheduler.schedule()` BACK requeue, schedule-remove pause/reinsert, nested switch/resume order, blocked direct-switch parent-chain unwind, and scheduler switch-trap no-mutation behavior are covered in core; non-main `scheduler.run()` and Python exception object/text details remain bridge coverage. |
...
| `capiTest/Tasklet.cpp::PyTasklet_Setup`, `PyTasklet_Insert`, `PyTasklet_Alive`, `PyTasklet_Kill` | core then ffi | Covered by the real legacy `Tasklet.cpp` source slice plus PyO3 bridge tests for simple setup/run callable workload, insert/kill, alive state, run-count updates, and refcount behavior around setup/kill. `PyTasklet_Alive`, dead-tasklet kill no-op behavior, timeout completion accounting, and paused direct-run eligibility now read `CoreScheduler` snapshots even if bridge-local lifecycle fields drift. The optional in-process source-slice probe currently fails after the first test due repeated embedded interpreter reinitialization, so full in-process lifecycle remains a broader C API binary blocker. |
...
| `capiTest/Scheduler.cpp::PyScheduler_Schedule`, `PyScheduler_GetRunCount`, `PyScheduler_GetCurrent`, `PyScheduler_RunNTasklets`, `PyScheduler_RunForTime` | core then ffi | Covered by the real legacy `Scheduler.cpp` source slice for current/run-count/run-n behavior and practical timeout pumping semantics; `PyScheduler_GetCurrent` now resolves through CoreScheduler per-run-queue current-tasklet identity, and core fixtures now include bounded run-n, limited schedule-order pumping, zero-timeout one-tasklet progress, remaining scheduled tasklets after an expired timeout, and large-timeout drain counters. |
...
| `capiTest/Scheduler.cpp::PyScheduler_GetNumberOfActiveScheduleManagers`, `PyScheduler_GetNumberOfActiveChannels`, `PyScheduler_GetAllTimeTaskletCount`, `PyScheduler_GetActiveTaskletCount`, `PyScheduler_GetTaskletsCompletedLastRunWithTimeout`, `PyScheduler_GetTaskletsSwitchedLastRunWithTimeout` | core then ffi | Covered by the real legacy `Scheduler.cpp` source slice for active manager/channel/tasklet counters and last-timeout completed/switched counters; public active/all-time tasklet counters, active schedule-manager counters, active channel counters, C API last-timeout counters, and nested-tasklet mode now read CoreScheduler state, and `scheduler_timeout_large_counts_all_tasklets` asserts the simple 3-completed/6-switched legacy timeout-counter case in core. |

## Scheduler Fixture Corpus

The scheduler fixture corpus lives in `fixtures/scheduler/`. It currently has
67 JSON fixtures: 40 with exact ordered event expectations, 67 with final-state
checks, 23 with trace-expectation checks, and two fixture-level teardown cleanup
checks. The latest promoted fixtures cover `scheduler.schedule()` BACK requeue
...
schedule/schedule_remove events. The corpus also covers nested parent/depth
metadata, nested `tasklet.switch()` parent-chain resume order, and blocked
parent-chain switch unwind after a channel wakeup. All run
...
operation, BACK requeue, FRONT_PLUS_ONE, parent-chain switch/resume, blocked
parent-chain switch unwind, and switch-trap operation boundaries. The nested
parent-chain fixtures check parent/depth metadata and resume ordering in both
trace events and final scheduler state.
These fixtures
