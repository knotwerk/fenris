...
54 JSON fixtures: 48 with ordered event expectations, one final-state timeout
snapshot, one fixture-level teardown cleanup check, and four trace-expectation
bounded-pump schedule-order checks. The latest promoted fixtures cover
`TestWithLimit`-style repeated `run_n_tasklets(1)` for single-level and
multi-level schedule order with nested tasklets enabled and disabled. All run
...
partial events and exact event match counts for the bounded pump boundaries.
These fixtures
